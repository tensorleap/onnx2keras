...
def test_albert_qa_huggingface():
    ...
    new_model = tf.keras.Model(inputs=flipped_model.inputs, outputs=flipped_model.layers[-5].output)
    flipped_otpt = flipped_model(input_np)
    assert np.abs((out[0]-flipped_otpt[0])).max() < 1e-04
    assert np.abs((out[1]-flipped_otpt[1])).max() < 1e-04
...
@pytest.mark.parametrize('aws_s3_download', [["inspired/data_meghna/", "", True]], indirect=True)
def test_inspired_megna_model(aws_s3_download):
    """Test conversion of asensus lung_anatomy_merged_ir9.onnx model"""
    model_path = f'{aws_s3_download}/model_0_fold.onnx'
    ...
    for i, (keras_out, onnx_out) in enumerate(zip(keras_outputs, onnx_outputs)):
        ...
        assert mean_error < 5e-6, f"Output {i} mean error {mean_error} exceeds threshold"
        assert max_error < 5e-5, f"Output {i} max error {max_error} exceeds threshold"

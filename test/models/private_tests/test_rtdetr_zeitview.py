...
ONNX_MODEL_PATH = "/Users/ranhomri/repos/hub/RT-DETR/client_format_structure.onnx"


def test_rtdetr_zeitview():
    ...
    input_arrays = {
        info.name: rng.random(
            [d if isinstance(d, int) and d > 0 else 1 for d in info.shape]
        ).astype(np.float32)
    ...
    for i, (keras_out, onnx_out) in enumerate(zip(keras_outputs, onnx_outputs)):
        ...
        assert keras_np.shape == onnx_out.shape, (
            f"Output {i} ({output_names[i]}) shape mismatch: {keras_np.shape} vs {onnx_out.shape}"
        )
        ...
        # RT-DETR outputs 300 fixed-position object queries; scores and logits
        # are query-aligned and compared element-wise. Boxes use sorted
        # comparison because small numeric differences in scores can shift
        # top-k ordering downstream.
        if output_names[i] == "boxes":
            ...
            diff = np.abs(keras_np.astype(np.float64) - onnx_out.astype(np.float64))

        mean_error = float(diff.mean())
        max_error = float(diff.max())
        assert mean_error < 1e-4, (
            f"Output {i} ({output_names[i]}) mean error {mean_error:.6f} exceeds threshold"
        )
        assert max_error < 1e-3, (
            f"Output {i} ({output_names[i]}) max error {max_error:.6f} exceeds threshold"
        )

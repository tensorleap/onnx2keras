...
def test_acosh():
    ...
    node = helper.make_node('Acosh', inputs=['x'], outputs=['y'])
    model = make_single_op_model('Acosh', [node], [x_info], [y_info], opset_version=9)
    x = np.random.uniform(1, 3, (2, 3, 4)).astype(np.float32)

...
def _make_reduce_sum_model_attr(axes, keepdims, opset_version):
    ...
    node = helper.make_node('ReduceSum', inputs=['data'], outputs=['reduced'],
                            axes=axes, keepdims=keepdims)
    return make_single_op_model('ReduceSum', [node], [data_info], [out_info], opset_version)


def _make_reduce_sum_model_input(axes, keepdims, opset_version):
    ...
    node = helper.make_node('ReduceSum', inputs=['data', 'axes'], outputs=['reduced'],
                            keepdims=keepdims)
    return make_single_op_model('ReduceSum', [node], [data_info], [out_info], opset_version,
...
def test_reduce_sum_axes_attr(opset_version):
    data = np.random.randn(2, 3, 4).astype(np.float32)
    model = _make_reduce_sum_model_attr([1], 1, opset_version)
    run_op_test(model, {'data': data}, ['data'])


@pytest.mark.parametrize('axes', ([0], [1]))
@pytest.mark.parametrize('opset_version', [13])
def test_reduce_sum_axes_input(axes, opset_version):
    data = np.random.randn(2, 3, 4).astype(np.float32)
    model = _make_reduce_sum_model_input(axes, 1, opset_version)
    run_op_test(model, {'data': data}, ['data'])

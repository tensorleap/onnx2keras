...
def _make_reduce_l2_model_attr(axes, keepdims, opset_version):
    ...
    node = helper.make_node('ReduceL2', inputs=['data'], outputs=['reduced'],
                            axes=axes, keepdims=keepdims)
    return make_single_op_model('ReduceL2', [node], [data_info], [out_info], opset_version)


def _make_reduce_l2_model_input(axes, keepdims, opset_version):
    data_info = helper.make_tensor_value_info('data', TensorProto.FLOAT, [2, 3, 4])
    out_info = helper.make_tensor_value_info('reduced', TensorProto.FLOAT, [2, 1, 4])
    axes_init = numpy_helper.from_array(np.array(axes, dtype=np.int64), name='axes')
    node = helper.make_node('ReduceL2', inputs=['data', 'axes'], outputs=['reduced'],
                            keepdims=keepdims)
    return make_single_op_model('ReduceL2', [node], [data_info], [out_info], opset_version,
...
def test_reduce_l2_axes_input(opset_version):
    data = np.random.randn(2, 3, 4).astype(np.float32)
    model = _make_reduce_l2_model_input([1], 1, opset_version)
    run_op_test(model, {'data': data}, ['data'])

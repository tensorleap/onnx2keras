...
import keras
import logging
from onnx2kerastl.utils import is_numpy, ensure_tf_type
    ...
    try:
        if not input_0_is_non_keras and not input_1_is_non_keras:
            ...
            if input_0.shape != input_1.shape and input_0.shape[:-1] == input_1.shape:
                to_add = tf.repeat(
                    tf.expand_dims(input_1, axis=-1), input_0.shape[-1], axis=-1
                )
...
#     inputs = list()
#     for i, inp in enumerate(node.input):
#         input_ = ensure_tf_type(layers[inp], name="%s_const%i" % (keras_name, i + 1))
...
def convert_where(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if layers[node.input[1]].dtype == np.int64 and is_numpy(layers[node.input[1]]):
        ...
        layers[node_name] = tf.where(
            tf.logical_not(casted), layers[node.input[2]], layers[node.input[1]]
        )
    else:
        layers[node_name] = tf.where(
            casted, layers[node.input[1]], layers[node.input[2]]
        )
...
def convert_bitshift(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if direction == "LEFT":
        ...
        raise AttributeError(
            "Onnx2Kerras cannot convert the BitShift operator"
            " since the 'direction' attribute was missing"
        )
    layers[node_name] = shifter_pointer(
        tf.cast(layers[node.input[0]], tf.uint64),
